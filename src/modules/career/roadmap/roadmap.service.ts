...
export class RoadmapService {
 ...
 async updateStep(userId: string, roadmapId: string, stepId: string, dto: UpdateStepDto): Promise<RoadmapEntity> {
  ...
  const isActiveStep = (s: RoadmapStep) =>
    s.status === StepStatus.IN_PROGRESS || s.status === StepStatus.COMPLETED;

  const isDoneStep = (s: RoadmapStep) =>
    s.status === StepStatus.COMPLETED || s.status === StepStatus.SKIPPED;

  const hasActiveStep = roadmap.steps.some(isActiveStep);
  const allDone = roadmap.steps.every(isDoneStep);

  // draft → active on first engagement
  if (roadmap.status === RoadmapStatus.DRAFT && hasActiveStep) {
    roadmap.status = RoadmapStatus.ACTIVE;
  }

  // active → completed when everything is done
  if (roadmap.status === RoadmapStatus.ACTIVE && allDone) {
    roadmap.status = RoadmapStatus.COMPLETED;
  }

  // completed → active if a step is later unmarked
  if (roadmap.status === RoadmapStatus.COMPLETED && !allDone) {
    roadmap.status = RoadmapStatus.ACTIVE;
  }

  // auto startDate — set once, never overwritten
  if (!roadmap.startDate && hasActiveStep) {
    roadmap.startDate = new Date();

    // also compute endDate from startDate + durationWeeks if not already set
    if (!roadmap.endDate && roadmap.generationParams?.durationWeeks) {
      roadmap.endDate = this.computeEndDate(roadmap.generationParams.durationWeeks);
    }
  }

  // auto endDate — stamp completion date or clear if reverted
  if (allDone) {
    roadmap.endDate = new Date();
  } else if (roadmap.status === RoadmapStatus.ACTIVE && roadmap.startDate) {
    // restore planned end date if it was overwritten by a completion that got reverted
    roadmap.endDate = roadmap.generationParams?.durationWeeks
      ? this.computeEndDateFrom(roadmap.startDate, roadmap.generationParams.durationWeeks)
      : null;
  }
  ...
  private computeEndDateFrom(from: Date, durationWeeks: number): Date {
  ...
  private async generate(userId: string, dto: CreateRoadmapDto) {
    ...
    const profileContext =
      profile && dto.mode !== RoadmapMode.GENERIC
        ? this.buildProfileContext(profile)
        : undefined;
  ...
  private buildProfileContext(profile: ProfileEntity) {
